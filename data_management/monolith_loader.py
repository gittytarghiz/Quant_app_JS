...
try:
    ...
    from data_management.eodhd_dl import eodhd_download   
except ImportError:
    ...
    from eodhd_dl import eodhd_download                   
...
def get_downloaded_series(tickers: List[str], start: str, end: str, dtype: str, interval: str = "1d") -> pd.DataFrame:
    ...
    for t, rng in {k:v for k,v in status.items() if v is not True}.items():
        ...
        # --- PRIMARY: EODHD ---
        try:
            ok, series = eodhd_download(t.replace("-", "."), s, e, interval, dtype)
        except Exception:
            ok, series = False, None
        ...
        if not ok or series is None or series.empty:
            try:
                ok, series = stooq_download(t.replace("-", ".") + ".US", s, e, interval, dtype)
            except Exception:
                ok, series = False, None
        if not ok or series is None or series.empty:
            try:
        ...
        if not ok or series is None or series.empty:
            try:
                ok, series = alpha_vantage_download(t.replace("-", "."), s, e, interval, dtype)
            except Exception:
                ok, series = False, None
        ...
        # Save to parquet
        series = series.squeeze(); series.name = dtype
        ...
        df = pd.concat([existing, series], axis=0)
