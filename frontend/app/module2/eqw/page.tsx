...
export default function EqwPage() {
  ...
  async function run() {
    ...
    try {
      ...
      const res = await fetch(`${API}/opt/equal-weight`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        signal: ctrl.signal,
        body: JSON.stringify({
          tickers: tickers.split(',').map(s => s.trim()).filter(Boolean),
          start, end, leverage: Number(lev),
      ...
      const json: EqwResponse = await res.json();
      setData(json);
    } catch (e: any) {
  ...
  // Removed legacy sample loader and unused columns derivation
  ...
  return (
    <div>
      <OptimizerNav />
      <h2>Equal Weight Tester</h2>
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
        <label>Tickers <input value={tickers} onChange={e => setTickers(e.target.value)} style={{ width: 240 }} /></label>
