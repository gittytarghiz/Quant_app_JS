...
export default function UserWeightsPage() {
  ...
  const [weights, setWeights] = useState("AAPL:0.125, MSFT:0.125, GOOGL:0.125, META:0.125, JNJ:0.125, GC=F:0.125, AMZN:0.125, NVDA:0.125");
  ...
  async function run() {
    ...
    try {
      const res = await fetch(`/opt/user-weights`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          tickers: tickers.split(',').map(s => s.trim().toUpperCase()).filter(Boolean),
          start, end, static_weights: parseWeights(weights), normalize: true, leverage: Number(lev)
        })
      });
      if (!res.ok) {
  ...
  return (
    <div>
      ...
      <div style={{ marginTop: 8 }}>
        <label>Static Weights <input value={weights} onChange={e => setWeights(e.target.value)} style={{ width: 600 }} /></label>
