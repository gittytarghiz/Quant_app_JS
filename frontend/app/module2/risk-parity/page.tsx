...
export default function RiskParityPage() {
  ...
  const [covEst, setCovEst] = useState<string>("sample");

  async function run() {
    setLoading(true); setErr(null);
    try {
      const res = await fetch(`/opt/risk-parity`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ tickers: tickers.split(',').map(s => s.trim()).filter(Boolean), start, end, leverage: Number(lev), min_weight: Number(minW), max_weight: Number(maxW), cov_estimator: covEst })
      });
      if (!res.ok) {
